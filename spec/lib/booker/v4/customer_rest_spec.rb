...
describe Booker::V4::CustomerREST do
  ...
  describe '#run_multi_spa_multi_sub_category_availability' do
    ...
    before do
      expect(client).to receive(:access_token).and_return 'access_token'
      expect(client).to receive(:post).with('/availability/multispamultisubcategory', expected_params, Booker::V4::Models::SpaEmployeeAvailabilitySearchItem).and_return([])
    end
  ...
  describe '#run_multi_service_availability' do
    ...
    before do
      expect(client).to receive(:access_token).and_return 'access_token'
      expect(client).to receive(:post).with('/availability/multiservice', expected_params, Booker::V4::Models::MultiServiceAvailabilityResult).and_return([])
    end
  ...
  describe '#run_class_availability' do
    ...
    before do
      expect(client).to receive(:access_token).and_return 'access_token'
      expect(client).to receive(:post).with('/availability/class', expected_params, Booker::V4::Models::ClassInstance).and_return([])
    end
